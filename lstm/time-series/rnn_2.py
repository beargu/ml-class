...
print(trainX.shape[0], trainX.shape[1], trainY.shape[0])

# create and fit the RNN
model = Sequential()
#model.add(SimpleRNN(1, input_shape=(config.look_back,1 ))) # one output
model.add(SimpleRNN(50, input_shape=(config.look_back,1 ))) # one output
#model.add(LSTM(5))
...
model.fit(trainX, trainY, epochs=1000, batch_size=2, validation_data=(testX, testY),  callbacks=[WandbCallback(), PlotCallback(trainX, trainY, testX, testY, config.look_back)])
